...
type: lecture
lecture: 10
tags: [closure, lexical-scope, environment, heap-allocation]
status: complete
---
...
- 闭包可在创建它的作用域结束后继续使用自由变量，因此这些变量所在的帧不能随控制栈弹出而消失。
- 链式栈把帧分配到堆上，并用父指针表示词法嵌套；闭包保存代码和创建时的环境指针。
- 函数调用会创建新帧，将实参与局部变量绑定到该帧，再沿父链解析自由变量；帧的回收交给 GC。
