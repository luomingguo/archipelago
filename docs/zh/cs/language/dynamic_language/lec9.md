...
type: lecture
lecture: 9
tags: [lexical-scope, closure, stack-frame, heap]
status: complete
---
...
- 作用域决定标识符引用指向哪个声明，闭包则是函数代码与它所捕获作用域的组合。
- 块作用域可以用帧栈表示：进入块压入空帧，`var` 在栈顶声明，普通赋值向下查找并更新已有绑定，退出块时弹帧。
- 帧保存地址、堆保存值的拆分让作用域更新与对象生命期分离，但纯栈式帧仍无法支持逃逸作用域。
