---
title: 6.1120 动态计算机语言工程
type: course
course: 6.1120 动态计算机语言工程
course_id: '6.1120'
tags: [dynamic-language, operational-semantics, virtual-machine, garbage-collection, jit-compilation]
status: complete
...
> MIT 6.1120 / 6.818 · Fall 2025 · Michael Carbin

## TL;DR

- 课程以 MITScript 为对象，从词法与解析出发，先用操作语义定义正确行为，再将语义降级为解释器、字节码 VM 和机器码。
- 堆、闭包与记录的表示选择会直接决定垃圾回收、变量查找和代码生成的成本。
- 后半程把类型、值、活跃性与逃逸分析统一到数据流 / 抽象解释框架，支撑 JIT、寄存器分配与推测优化。

## 资料来源与整理说明

- 课程定位与课号以 [MIT 课程目录](https://catalog.mit.edu/subjects/6/) 为准。原 2025 课程站 `6112-fa25.github.io` 当前已下线，本仓库也没有保存原始 transcript 或字幕文件。
- 现有 24 讲中文笔记已把课堂讲义主线与补充阅读整合为知识结构；本轮保留这些可追溯内容，不伪造无法核验的视频原话。
- 若后续补入可公开访问的视频或 transcript，应以 transcript 补足教师口头解释、课堂例子与强调点，同时以 lecture notes 核对公式、伪代码和术语。
...
最后一节课是“虚拟机对抗赛”，各组比较生成代码的运行速度；评估强调通用优化，不允许针对比赛程序写特例 hack。

## 讲义目录

| 单元 | 讲义 | 学习主线 |
|---|---|---|
| 全景与词法 | [L1 课程概览](./lec1.md) · [L2 词法分析](./lec2.md) | 动态语言的成本与 lexer |
| 语法分析 | [L3 基本概念](./lec3.md) · [L4 自顶向下解析](./lec4.md) · [L5 预测分析与 AST](./lec5.md) | CFG、歧义、First 集与递归下降 |
| 操作语义 | [L6 语义导论](./lec6.md) · [L7 IMP](./lec7.md) · [L8 堆与记录](./lec8.md) | 从求值关系到带堆的执行状态 |
| 作用域与闭包 | [L9 帧栈](./lec9.md) · [L10 链式栈](./lec10.md) · [L11 作用域语义](./lec11.md) | 词法作用域、自由变量与对象表示 |
| 垃圾回收 | [L12 可达性与引用计数](./lec12.md) · [L13 标记-清扫与复制](./lec13.md) · [L14 分代与增量](./lec14.md) | 吞吐、暂停、空间与精度的取舍 |
| 字节码 VM | [L15 VM 动机](./lec15.md) · [L16 语法制导翻译](./lec16.md) · [L17 表示与符号表](./lec17.md) | 将 AST 降级为可高效解释的表示 |
| 机器码与优化 | [L18 代码生成动机](./lec18.md) · [L19 IR 与分析](./lec19.md) · [L20 x86-64 代码生成](./lec20.md) | AOT / JIT、部分求值与运行时特化 |
| 寄存器分配 | [L21 活跃性](./lec21.md) · [L22 线性扫描与图着色](./lec22.md) | 用程序点上的活跃事实管理稀缺寄存器 |
| 静态分析 | [L23 数据流框架](./lec23.md) · [L24 格与不动点](./lec24.md) | 用格、单调函数与迭代统一课程中的分析 |
