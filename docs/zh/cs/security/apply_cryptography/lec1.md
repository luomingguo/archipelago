...
type: lecture
lecture: 1
tags: [cryptographic-hash, one-way-function, collision-resistance, random-oracle]
status: complete
source: 'https://65610.csail.mit.edu/2026/lec/l01-hashing.pdf'
---
# Lec 1 加密哈希函数与单向性
...
- 随机预言机把每个新输入映射为独立均匀输出，是分析哈希协议的理想模型；现实中的 SHA-2/SHA-3 只是具体、公开的替代函数。
- 单向性、抗碰撞与目标抗碰撞回答不同攻击问题，彼此不能随意替换；选择性质必须从上层协议真正需要阻止的攻击出发。
- 对 $d$ 位输出，通用原像搜索约需 $2^d$ 次查询，碰撞搜索受生日界限制约需 $2^{d/2}$ 次，因此碰撞安全位数约为输出长度的一半。
...
::: insight
抗碰撞、单向性和伪随机性回答的是不同问题；把一种性质代替另一种使用，是密码协议常见的设计错误。
:::
