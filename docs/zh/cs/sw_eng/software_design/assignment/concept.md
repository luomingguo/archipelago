...
title: 阅读与编写概念规格
type: assignment
tags: [concept-design, concept-specification, state-modeling, authentication]
status: complete
source: 'https://61040-fa25.github.io/assignments/problem-set-1'
---
# 阅读与编写概念规格
...
- 作业通过阅读、补全、对比和从零设计四类练习，训练从 purpose、operational principle、state 与 actions 判断概念规格是否自洽。
- `GiftRegistration` 与 `PasswordAuthentication` 练习要求从状态、不变量和前后条件推断行为，而不是只复述字段或界面流程。
- 对比密码认证与个人访问令牌说明：相似问题可以由不同概念解决，规格应解释选择背后的产品意图和边界。
- 最终设计题要求为三个真实机制写完整规格，并用 rubric 检查非显然决策；LLM 可以辅助审阅，但不能替代设计判断。
...
::: insight
写概念规格时，最能暴露问题的不是补齐字段，而是尝试从规格回答反事实问题：前置条件不满足会怎样、两个动作换序是否改变结果、状态能否表达所有承诺。若这些问题只能靠想象 UI 或实现代码才能回答，规格仍混入了未声明的假设。
:::
